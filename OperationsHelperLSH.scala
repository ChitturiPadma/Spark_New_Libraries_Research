...
class OperationsHelperLSH(_ds: DataFrame,
  ...
  /*def preparedDataSet()(df: DataFrame): DataFrame = {
    df.na.fill("0").withColumn(
      ...
      trim(concat(trim(col("allfeat_source_attribute_area")),
        lit(" "),
        trim(col("allfeat_matched_attribute_area")),
        lit(" "),
        trim(col("allfeat_indoor_calc")),
        lit(" "),
        trim(col("allfeat_indoor_match_calc")),
        lit(" "),
        trim(col("allfeat_bedroom_match_calc")),
        lit(" "),
        trim(col("allfeat_bathroom_match_calc")),

        lit(" "),
        trim(col("allfeat_kitchen_match_calc")),

        lit(" "),
        trim(col("allfeat_yard_match_calc")),

        lit(" "),
        trim(col("allfeat_landscape_match_calc")),

        lit(" "),
        trim(col("allfeat_livingroom_match_calc")),

        lit(" "),
        trim(col("allfeat_others_in_match_calc")),

        lit(" "),
        trim(col("allfeat_others_out_match_calc")),


        lit(" "),
        trim(col("allfeat_otherrooms_match_calc")),

        lit(" "),
        trim(col("allfeat_exterior_match_calc")),

        lit(" "),
        trim(col("allfeat_diningroom_match_calc")),

        lit(" "),
        trim(col("allfeat_swimming_match_calc")),


        lit(" "),
        trim(col("allfeat_balcony_match_calc")),


        lit(" "),
        trim(col("allfeat_bathroom_diff")),

        lit(" "),
        trim(col("allfeat_bedroom_diff")),

        lit(" "),
        trim(col("allfeat_image_match_percent")),

        lit(" "),
        trim(col("allfeat_indoor_match_percent")),


        lit(" "),
        trim(col("allfeat_latlong_distance_unit")),

        lit(" "),
        trim(col("allfeat_matched_property_title")),


        lit(" "),
        trim(col("allfeat_sleeps_diff")),

        lit(" "),
        trim(col("allfeat_source_property_title"))
  ...
  def getWordsFilteredDF(df: DataFrame): (DataFrame, CountVectorizerModel) = {
    ...
    val vectorizedProductsDF = vectorModeler.transform(wordsFiltered)
      ...
      .select(col(OperationsHelperLSH.ConcatComments),
